    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const SheetInterface& sheet) const override {
                double result = 0.;
                switch (type_) {
                case UnaryOpExpr::UnaryPlus:
                    result = operand_->Evaluate(sheet);
                    break;
                case UnaryOpExpr::UnaryMinus:
                    result = -1. * operand_->Evaluate(sheet);
                    break;
                }
                return result;
            }
        ...
        public:
            ...
            double Evaluate(const SheetInterface& sheet) const override {
                ...
                auto cell_ptr = sheet.GetCell(*cell_);
                // если по адрессу нет ячейки, возвращаем ноль
                if (cell_ptr == nullptr) {
                    return 0.;
                }
                ...
                if (std::holds_alternative<std::string>(cell_value)) {
                    ...
                    else if (std::all_of(str_value.begin(), str_value.end(), ::isdigit)) {
